...
/*   Updated: 2025/04/10 17:51:33 by layang           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_shift_in_out(t_token	**redir, t_token_type type)
{
	...
	*redir = (*redir)->next;
	...
	else if (type == REDIRECT_OUT)
	{
		...
		printf("redirect outfile to: %s, fd: %d.\n", (*redir)->value, (*redir)->outfile);// ** test code
	}
	else if (type == APPEND)
	{
		(*redir)->outfile = open((*redir)->value, O_WRONLY | O_CREAT
			| O_APPEND, 0644);
		ft_file_ctr((*redir)->outfile, "minishell: open failed");
		printf("append outfile to: %s, fd: %d.\n", (*redir)->value, (*redir)->outfile);// ** test code
	}
	ft_pass_in_out(redir);
	*redir = (*redir)->next;
}
...
static void	ft_do_here_doc(t_token	**heredoc, int heredoc_fd)
{
	...
	while (1)
	{
		...
		if (!line)  // get next line failed（Ctrl+D）
			ft_file_ctr(-2, "minishell: get next line failed");
		len = ft_strlen((*heredoc)->value);
		if (ft_strncmp(line, (*heredoc)->value, len) == 0
			&& line[len] == '\n')
		{
			free(line);
			get_next_line(-1);
