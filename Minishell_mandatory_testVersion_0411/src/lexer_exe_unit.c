...
void ft_fill_tab(t_mini *mini)
{
	...
	while (i < mini->tab_size)
	{
		...
		{		
			printf("tmp address: %p, value: %s\n", (void *)tmp, tmp->value);// ** test code
			new_token = ft_create_token(tmp->value, tmp->type);    //add really new one/
            ft_add_token(&mini->exe_tab[i], new_token);
			printf("add line %d: %s\n\n", i, tmp->value);        // ** test code
			tmp = tmp->next;	
		}
		if (tmp && tmp->type == PIPE)
		{
			printf("\nSkipping PIPE, tmp address: %p\n", (void *)tmp); // ** test code
			tmp = tmp->next;
		}
		i++;
		printf("*i is: %d\n", i);                                    // ** test code
