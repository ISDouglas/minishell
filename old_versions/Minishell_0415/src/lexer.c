...
/*   Updated: 2025/04/15 17:21:36 by nimorel          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_lexer(t_mini	*mini)
{
	...
	while (mini->input[i])
	{
		if (ft_isspace(mini->input[i]))
			...
			ft_handle_dquote(mini->input, &i, &mini->lexer, &mini->env);
		else if (mini->input[i] == '$')
			ft_handle_env_var(mini->input, &i, &mini->lexer);
		else
			ft_handle_word(mini->input, &i, &mini->lexer, &mini->env);
	}
	printf("\n**add lexer succeed:\n");
